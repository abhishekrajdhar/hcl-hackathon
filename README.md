...
and CI with no API key and no torch. One substitution: `LLM_PROVIDER=openai`
with no `OPENAI_API_KEY` uses **Claude** instead when an `ANTHROPIC_API_KEY`
is set — the operator asked for a real model and has the credentials for one.
The swap is logged and `/health` reports the provider actually answering.
...
handling. There is no bundled dataset anywhere: `/dashboard?demo=1` signs into
the seeded **demo learner** (`DEMO_LEARNER_EMAIL`/`_PASSWORD`, defaults
`demo@example.com` / `demo-universe`), a real account whose profile, roadmap
and progress were generated by the actual engines at seed time — so the demo
is served live by the same API path as any other learner, coach included. The
landing hero fetches the same account's universe through the unauthenticated
read-only `GET /public/demo/universe`.
...
dashboard with ten sections — Learning Universe, Overview, Current Mission,
...
`/dashboard?demo=1` silently signs into the shared demo learner — live data,
clearly bannered as not the visitor's own.
